...
void stampTimeFormat(UINT8* text,UINT8 hour,UINT8 minute,UINT8 second,UINT8 fmt)
{
	...
	switch(fmt){
	case 0: pattern[11] = 0; break;
	case 1: ai = 12; pattern[11] = ' '; pattern[12] = 'A'; pattern[13] = 'M'; pattern[14] = 0; break;
	case 2: pattern[17] = 0; break;
	case 3: ai = 18; break;
	case 4: ai = 18; pattern[19] = 0; break;	
	case 5: ai = 12; pattern[11] = ' '; pattern[12] = 'A'; pattern[13] = 0; break;
	default: pattern[17] = 0; break;
	}
	if(ai && hour>=12){	
...
void stampDateFormat(UINT8* text,UINT8 year,UINT8 mon,UINT8 day,UINT8 fmt)
{
	...
	switch( fmt ){
	case 0: sprintf(text,pattern0,year,mon,day); break;
	case 1: sprintf(text,pattern1,day,mon,year); break;
	case 2: sprintf(text,pattern1,mon,day,year); break;
	case 3: sprintf(text,pattern2,year,mon,day); break;
	default: sprintf(text,pattern0,year,mon,day); break;
	}
}
...
static void stampAutoScale(UINT16 w, UINT16 h)
{
	...
	// 消灭警告的小技巧：即便没有用到，也象征性地读一下避免编译器报错
	actualH = h;
