    ...
    for (r = 0; r < DOT_ROWS; r++) {
        for (c = 0; c < DOT_COLS; c++) {
            if (pattern[r] & (0x10 >> c)) {
                ...
                if(xPos + dotSize > w || yPos + dotSize > h) continue;
	...
	if(ptText->imgAddrW){
		...
		if(w+ptText->xOff<=stampBGWidth){
            ...
            if(gStampVideoMode == 1) {
                ...
                for(i=0; i<c; i++) {
                    // 核心修复二：去掉了原版 && text[i]。
                    // 保证即使是第一次刷新（text[i]为空），也能正常把字模拷入缓冲，洗刷掉灰阶背景！
                    if((text[i] != str[i]) && str[i]){
...
void stampTextModifyByRTC(dateStc_t* rtc)
{
    ...
    // 模式隔离核心修复二：
    if(gStampVideoMode == 1) {
        // 录像模式：保持原版多行独立显示（DATE与TIME分离）
        if(ptDate->enable){
            stampDateFormat(dateStr, rtc->Year, rtc->Month, rtc->Day, stampDateFmt);
            stampTextModifyByString(STAMP_DATE, dateStr);
        }
        if(ptTime->enable){
            stampTimeFormat(timeStr, rtc->Hour, rtc->Minute, rtc->Second, stampTimeFmt);
            stampTextModifyByString(STAMP_TIME, timeStr);
        }	
    } else {
        // 拍照模式：合并成一行（STAMP_DATE容纳整体，禁用STAMP_TIME）
        stampDateFormat(dateStr, rtc->Year, rtc->Month, rtc->Day, stampDateFmt);
        stampTimeFormat(timeStr, rtc->Hour, rtc->Minute, rtc->Second, stampTimeFmt);
        sprintf((char*)finalStr,"%s %s",dateStr,timeStr);
        
        if(ptDate->enable){
            stampTextModifyByString(STAMP_DATE, finalStr);
        }
        if(ptTime->enable){
            ptTime->enable = 0; // 仅限拍照模式禁用 TIME 行
        }
    }
}
	...
	for(i=0;i<STAMP_TYPE_MAX;i++)
	{
		...
		if(ptText->imgAddrW){
			HAL_GprmCopy(
				...
				, dstAddrW
				, stampBGWidth, stampBGHeight, ptText->xOff, ptText->yOff
...
{
	// 核心修复一：在初始化最前面，强制系统切入录像模式，防止点阵逻辑污染录像缓冲！
	stampVideoModeSet(1); 
	...
	stampTextSet(STAMP_TIME, 0, 48, 5); // 确保加上时间的 Y 轴偏移
	
	stampTextEnable(STAMP_DATE, 1);
	stampTextEnable(STAMP_TIME, 1);     // 确保开启第二行显示
